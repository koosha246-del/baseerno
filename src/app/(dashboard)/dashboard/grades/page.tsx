...
export default async function GradesPage() {
  ...
  // Fetch grades + courses in parallel, then only fetch students that
  // actually appear in the grades list (instead of all STUDENT users).
  const grades = await repository.listGrades(undefined, user.id);
  const courses = await repository.listCourses({ mentorId: user.id });

  // Targeted student lookup — only fetch the ones who have grades
  // from this teacher, instead of pulling every student in the system.
  const neededStudentIds = Array.from(new Set(grades.map((g) => g.userId)));
  const students = neededStudentIds.length > 0
    ? await repository.listUsers({ ids: neededStudentIds })
    : [];
